...
class Punch {
  constructor(userData, onChange) {
    ...
    this.lastSync = 0;
    this.inflight = null; // promise for the punch action currently hitting the server
  }

  /** Resolves once any in-flight clock action has settled. Quit awaits this so a
   *  just-clicked clock-out isn't killed mid-request. Never rejects. */
  async settle() {
    try { await this.inflight; } catch { /* the action itself handled failure */ }
  }
  ...
  /** Worked ms today billed to a specific (client id, project) bucket —
   *  General is clientId null. Mirrors clientTotalsToday's midnight/break
   *  math; project null matches any project under the client. */
  bucketMsToday(clientId, project, now = Date.now()) {
    const midnight = new Date(now).setHours(0, 0, 0, 0);
    let total = 0;
    for (const ps of Object.values(this.days).flat()) {
      if ((ps.client?.id ?? null) !== (clientId ?? null)) continue;
      if (project != null && (ps.project ?? null) !== project) continue;
  ...
  async sync() {
    const today = api.localDate();
    // Remember the session we BELIEVE is open, so a sync that comes back with
    // it closed-and-auto reveals a server-side closure (dead-machine watchdog,
    // 12h cap) — the panel raises the "you were clocked out" banner for it.
    const before = this.openSession();
    const res = await api.getPunch(prevDate(today), today);
    if (res.ok) {
      this.days = res.body.days ?? {};
      this.needsLogin = false;
      this.lastSync = Date.now();
      if (before && this.onAutoClosedRemotely) {
        const match = (this.days[before.date] ?? []).find((ps) => ps.in === before.in);
        if (match && match.out != null && match.auto) {
          this.onAutoClosedRemotely({ outAt: match.out, client: before.client ?? null, project: before.project ?? null });
        }
      }
      this.onChange();
  ...
  async act(action, opts = {}) {
    // Expose this call as the in-flight action (await-able, never rejects) so
    // quit can wait for delivery. Latest action wins; settle() awaits it.
    const p = this._act(action, opts);
    this.inflight = p.catch(() => {});
    return p;
  }

  async _act(action, opts = {}) {
    const now = Date.now();
    ...
    // A definitive server rejection (4xx validation — e.g. "client requires a
    // project") is NOT retryable: queueing it would wedge the replay queue
    // behind an action that will never succeed. Resync so the optimistic local
    // change rolls back to server truth, and surface the reason.
    if (typeof res.status === "number" && res.status >= 400 && res.status < 500) {
      await this.sync();
      return { rejected: true, error: res.error ?? "The server rejected that action." };
    }
  ...
  async drain() {
    ...
    // Replay in order. Definitive 4xx rejections are DROPPED (they'll never
    // succeed and would block everything behind them); transient failures stop
    // the drain and keep the remainder queued for next time.
    const remaining = [];
    let stopped = false;
    let progressed = false;
    for (const it of q) {
      if (stopped) { remaining.push(it); continue; }
      const res = await api.punch(it.date, it.action, it.opts);
      if (res.ok) { progressed = true; continue; }
      if (typeof res.status === "number" && res.status >= 400 && res.status < 500) {
        console.log(`[sns-desk] dropped rejected queued punch (${it.action}): ${res.error ?? res.status}`);
        continue;
      }
      if (res.needsLogin) this.needsLogin = true;
      stopped = true;
