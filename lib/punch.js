...
class Punch {
  ...
  applyLocal(action, opts, now) {
    ...
    if (action === "break") {
      ...
      // opts.general = de-attribute (switch to the neutral General bucket).
      if ((opts.client?.id && real.client?.id !== opts.client.id) || (opts.general && real.client)) {
        for (const b of real.breaks) if (b.e == null) b.e = now;
        real.out = now;
        day.push({ in: now, out: null, breaks: [], ...(opts.client?.id ? { client: opts.client } : {}) });
      }
    }
  }
