...
// Idle gate: base comes from the manager-set S&S idle minutes (deps.idleBaseMin),
// which is the TRUE MINIMUM — nobody is ever cut off before the configured
// allowance. The effective cutoff is randomized per idle episode within
// 100–140% of it, so the exact moment past the promise is unpredictable.
const IDLE_GATE_SPREAD = 0.4;
// The tracker never records itself ("Electron" is the dev-mode shell name),
// nor macOS system surfaces like the lock screen.
const SELF_APPS = ["S&S Desk", "Electron", "loginwindow", "ScreenSaverEngine", "SecurityAgent"];
...
class Tracker {
   ...
   *   idleBaseMin: () => number,          // manager-set S&S idle minutes (ceiling)
   * }
   */
  constructor(deps) {
    this.deps = deps;
    ...
    this.idleGateS = null;   // current effective idle cutoff (seconds)
    this.idleGated = false;  // true while an idle episode has closed the block
  }

  /** Roll a fresh effective idle cutoff: 100–140% of the manager-set minutes. */
  rollIdleGate() {
    const baseMin = Math.max(2, this.deps.idleBaseMin ? this.deps.idleBaseMin() : 10);
    this.idleGateS = Math.round(baseMin * 60 * (1 + IDLE_GATE_SPREAD * Math.random()));
  }
  ...
  async sample() {
    ...
    if (this.idleGateS == null) this.rollIdleGate();
    if (this.deps.getIdleSeconds() >= this.idleGateS) {
      // Idle past the (randomized) cutoff: close any running block at its
      // last active moment and record nothing until activity resumes.
      if (this.seg) this.flush();
      this.idleGated = true;
      return;
    }
    if (this.idleGated) {
      // Back from an idle episode — re-roll so the next cutoff is unpredictable.
