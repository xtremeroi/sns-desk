...
//
// Every block lands in the LOCAL day log (personal view, always on). Blocks
// are ALSO posted to the S&S timesheet, but only those that overlap punched-in
// time — off-the-clock activity stays on this machine.
...
class Tracker {
  ...
  async sample() {
    ...
    if (!name || this.deps.excludeApps().includes(name)) {
      if (this.seg) this.flush();
      return;
    }
