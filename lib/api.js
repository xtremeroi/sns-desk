...
// Desk identifies itself so the server can track "capture surface alive" —
// powers the dead-machine watchdog that closes sessions left open by a
// power-off or crash. Browsers reading the same endpoints don't send this.
const DESK_HEADER = { "x-sns-desk": (() => { try { return app.getVersion(); } catch { return "dev"; } })() };
...
async function call(url, init = {}) {
  ...
  try {
    res = await net.fetch(url, {
      ...
      redirect: "manual",
      ...init,
      headers: { accept: "application/json", ...DESK_HEADER, ...(init.headers ?? {}) },
    });
  } catch (e) {
...
/** Week-to-date worked vs management-allocated hours, per client:
 *  { weekStart, items: [{ id, n, hours (allocated), clocked (worked), status }] }. */
function getBudgetStatus(who, todayLocal) {
  return call(`${STORE}?budgetStatus=${encodeURIComponent(who)}&today=${todayLocal}`);
}
 ...
 *          reason ("idle"|"lock"|"off" — WHY an auto outAt fired; stored on the
 *          session server-side so the web Time page can say why) }
 */
function punch(dateLocal, action, opts = {}) {
  ...
  if (action === "outAt") {
    ...
    if (opts.reason) payload.reason = String(opts.reason).slice(0, 8);
...
module.exports = { BASE, getGlobal, getPunch, getTime, getLeave, getClientProjects, getBudgetStatus, punch, putTimeSegment, localDate };
